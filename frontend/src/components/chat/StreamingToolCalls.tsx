...
// Tool icon mapping (SVG, matching ToolCallCard)
...
// Status configuration (matching ToolCallCard)
...
/**
 * Component for displaying tool calls during streaming
 */
export function StreamingToolCalls({ toolCalls, onRemove }: StreamingToolCallsProps) {
  return (
    <div className="space-y-2 my-1.5">
      {toolCalls.map((toolCall) => (
        <StreamingToolCallCard
          key={toolCall.id}
          toolCall={toolCall}
          onComplete={() => onRemove?.(toolCall.id)}
        />
      ))}
    </div>
  )
...
function StreamingToolCallCard({ toolCall }: StreamingToolCallCardProps) {
  ...
  useEffect(() => {
    if (toolCall.status === 'running' && toolCall.startTime) {
      const interval = setInterval(() => {
        setElapsedTime(
          Math.round((Date.now() - toolCall.startTime!.getTime()) / 1000)
        )
      }, 1000)
  ...
  // Running tools default expanded, completed tools default collapsed
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Duration - show elapsed time for running, final duration for completed */}
          {(toolCall.status === 'running' || toolCall.endTime) && (
            <span
              style={{
                fontSize: '13px',
                color: '#999',
            ...
            >
              {toolCall.status === 'running' ? `${elapsedTime}s` :
      ...
      {/* Parameters - inline format (always show when there are arguments) */}
      ...
      {/* Error Display */}
      {toolCall.status === 'error' && toolCall.error && isExpanded && (
        <div
          style={{
            marginTop: '6px',
            background: '#fef2f2',
      ...
      {/* Timeout Message */}
      {toolCall.status === 'timeout' && isExpanded && (
        <div
          style={{
            marginTop: '6px',
            background: '#fffbeb',
      ...
      {/* Result Section - Expandable (only for completed tools) */}
      {(toolCall.status === 'success' || (toolCall.status !== 'running' && toolCall.result)) && (
...
export default StreamingToolCalls
